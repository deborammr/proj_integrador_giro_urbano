...
O repositório está organizado em diretórios separados para o cliente (Front-end) e para a API (Back-end):

```text
proj_transporte_urbano/
├── backend/       # API REST (Java, Spring Boot, H2 Database, JdbcTemplate)
└── frontend/      # Cliente Web (React, React Router DOM, Axios, CSS Modules)
...
Navegue até a pasta do back-end ou abra-a em sua IDE Java de preferência (IntelliJ, Eclipse, VS Code).

Certifique-se de que as dependências do Maven estão carregadas.

Execute a aplicação Spring Boot utilizando o terminal:
...
Abra um novo terminal na raiz do repositório e navegue até a pasta do front-end:

Bash
cd frontend
Instale as dependências padrão do projeto listadas no package.json (incluindo o Axios e o React Router DOM):
...
Inicie a aplicação front-end em modo de desenvolvimento (usando o Vite):
